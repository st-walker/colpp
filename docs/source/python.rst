...
The :code:`module/colpy` subfolder contains all the :code:`*.py` files of the module.
...
This will build the :code:`*.cpp` files in the :code:`src` folder and copy the folder :code:`module/colpy` folder to build location of the python module, namely :code:`${CMAKE_BINARY_DIR}/python/module/` where :code:`${CMAKE_BINARY_DIR}` is the  build directory.
...
Nevertheless we prefer to have a regular Python package with a proper :code:`__init__.py`. From the :code:`__init__.py` we import all the C++ / pybind11_ exported functionality from the build submodule named :code:`_colpy`.
...
.. code-block:: cpp
    ...
    PYBIND11_MODULE(_colpy , module)
    {
        ...
        col::def_new_stuff(module);  // <- our new functionality
...
.. code-block:: python
    ...
    colpy.new_stuff(numpy.arange(5), dtype='float64')
...
:code:`module/colpy` subfolder.
After adding the new file, cmake needs to be rerun since we copy the content :code:`module/colpy` during the build process.
